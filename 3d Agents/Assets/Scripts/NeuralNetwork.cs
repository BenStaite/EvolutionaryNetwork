...
public class NeuralNetwork : IComparable<NeuralNetwork>
{
    ...
    public NeuralNetwork(NeuralNetwork copyNetwork, bool carbonCopy)
    {
        kills = 0;
        damage = 0;
        damageTaken = 0;
        if (carbonCopy)
        {
            hasBeenBest = copyNetwork.hasBeenBest;
        }
        else
        {
            hasBeenBest = false;
        ...
        if (carbonCopy)
        {
            color = copyNetwork.color;
        }
        else
        {
            color = (copyNetwork.color + new Color(UnityEngine.Random.Range(0f, 1f), UnityEngine.Random.Range(0f, 1f), UnityEngine.Random.Range(0f, 1f)))/2;
        }
    ...
    /// Deep copy constructor 
    /// </summary>
    /// <param name="breedNetwork">Network to deep copy</param>
    public NeuralNetwork(NeuralNetwork netDad, NeuralNetwork netMom)
    {
        kills = 0;
        fitness = 0f;
        this.layers = new int[netDad.layers.Length];
        for (int i = 0; i < netDad.layers.Length; i++)
        {
            this.layers[i] = netDad.layers[i];
        }
        color = (netDad.color + netMom.color+ new Color(UnityEngine.Random.Range(0f, 1f), UnityEngine.Random.Range(0f, 1f), UnityEngine.Random.Range(0f, 1f)))/3;

        InitNeurons();
        InitWeights();
        BreedWeights(netDad.weights, netMom.weights);
    }

    private void BreedWeights(float[][][] dadWeights, float[][][] momWeights)
    {
        for (int i = 0; i < weights.Length; i++)
        {
            for (int j = 0; j < weights[i].Length; j++)
            {
                ...
                {
                    //weights[i][j][k] = (dadWeights[i][j][k]+ momWeights[i][j][k])/2f;
                    
                    if (UnityEngine.Random.Range(0f, 1f) > 0.5)
                    {
                        weights[i][j][k] = dadWeights[i][j][k];
                    }
                    else
                    {
                        weights[i][j][k] = momWeights[i][j][k];
                    }
                }
            }
        }
        ...
        for (int i = 1; i < layers.Length; i++)
        {
            ...
            for (int j = 0; j < neurons[i].Length; j++)
            {
                ...
                for (int k = 0; k < neuronsInPreviousLayer; k++)
                {
                    ...
                    //Debug.Log(neuronWeights[k].ToString());
                    //neuronWeights[k] = 0;
                }
    ...
    /// </summary>
    public void Mutate(float mutationMod)
    {
        for (int i = 0; i < weights.Length; i++)
        {
            for (int j = 0; j < weights[i].Length; j++)
            {
                for (int k = 0; k < weights[i][j].Length; k++)
                {
                    ...
                    //mutate weight value 
                    float randomNumber = UnityEngine.Random.Range(0f, mutationMod);
